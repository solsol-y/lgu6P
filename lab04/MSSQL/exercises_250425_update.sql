...
FROM (
	SELECT DISTINCT customerid
	FROM sales
	WHERE stockcode IN ('21730', '21615')          -- 특정 제품을 구매한 코드
		AND CONVERT(DATE, invoicedate) BETWEEN '2010-12-01' AND '2010-12-18'
	) s
LEFT 
JOIN (
...
-- 강사 고민 중 : 400 대신에 ATV 평균 집어넣어서 필터링
-- WITH 절 하는 방식, 인라인 뷰 사용해서 하는 방식 
...
-- 기본적인 질문 : 이 회사가 성장중? 
-- 재구매가 얼마나 빈번하게 자주 일어나는가? 
-- 재구매의 평균적인 금액이 계속 증가하는가? 
